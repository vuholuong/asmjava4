...
<body data-bs-spy="scroll" data-bs-target="#navbar-example2"
	...
	<section id="residence">
		<div class="container mt-5 py-5">
			<h2 class="text-capitalize m-0 py-lg-5">Danh Mục Đề Xuất</h2>
			<div
				class="swiper-button-next residence-swiper-next  residence-arrow"></div>
			<div class="swiper-button-prev residence-swiper-prev residence-arrow"></div>

			<div class="swiper residence-swiper">
				<div class="swiper-wrapper">
					<!-- Hiển thị các video có lượt xem cao nhất -->
					<%
					List<Video> topVideos = (List<Video>) request.getAttribute("topVideos");
					%>
					<%
									...
									<ul class="d-flex">
										<form action="LikeServlet" method="post" class="btn-info">
											...
											<button type="submit" class="btn text-light">
												<i class="bi bi-hand-thumbs-up"></i> LIKE &nbsp
											</button>
										</form>
										...
										<span class="residence-list"> &nbsp <%=video.getViews()%><i
											class="bi bi-eye"></i>
										</span>
									</ul>
								</div>
		...
		<div class="residence-btn">
			<a href="${pageContext.request.contextPath}/videoall"
				class="btn btn-primary btn-lg my-5">View All</a>
		</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title text-dark" id="shareModalLabel">Share
						Video</h5>
				...
				<div class="modal-body">
					<form id="shareForm" action="ShareServlet" method="post">
						...
						<div class="mb-3">
							<label for="message" class="form-label text-dark">Lời
								Nhắn</label>
	...
	<script>
		$(document).ready(function() {
			...
			$('.poster').click(function() {
				...
				window.location.href = '/video-detail-page';
				// Ghi nhận tiểu phẩm đã xem
				$.ajax({
					url : '/mark-as-viewed',
					type : 'POST',
					data : {
					...
					success : function(response) {
						console.log('Đã ghi nhận tiểu phẩm đã xem');
					}
				});
			});
			...
			$('.like-btn').click(function() {
				// Ghi nhận tiểu phẩm yêu thích
				$.ajax({
					url : '/mark-as-favorite',
					type : 'POST',
					data : {
						videoId : 'video-id'
					},
					success : function(response) {
						console.log('Đã ghi nhận tiểu phẩm yêu thích');
					}
				});
			});
			...
			$('.share-btn').click(function() {
				// Lấy địa chỉ email để chia sẻ
				let email = prompt("Nhập địa chỉ email của bạn bè:");
				if (email) {
					$.ajax({
						url : '/share-video',
						type : 'POST',
						...
						success : function(response) {
							console.log('Đã gửi liên kết video');
						}
					});
				}
			});
